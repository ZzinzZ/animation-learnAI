...
const AssignmentCard = ({ assignment, index, onCardRef, onStartTest }: AssignmentCardProps) => {
  ...
  const handleStartTest = () => {
    if (onStartTest && (assignment.type === "exam" || assignment.type === "quiz")) {
    ...
    >
      <Card className="relative overflow-hidden rounded-xl backdrop-blur-xl bg-white/25 border border-white/20 shadow-2xl">
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="floating-particle absolute -top-2 -right-2 w-4 h-4 bg-gradient-to-r from-purple-400 to-pink-400 rounded-full opacity-30" />
          <div className="floating-particle absolute top-1/2 -left-1 w-2 h-2 bg-gradient-to-r from-blue-400 to-cyan-400 rounded-full opacity-30" />
          <div className="floating-particle absolute bottom-4 right-1/3 w-3 h-3 bg-gradient-to-r from-green-400 to-emerald-400 rounded-full opacity-30" />
        </div>
          ...
          <div className="flex items-start justify-between">
            <div className="flex-1 space-y-2">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-lg text-black/70">{assignment.title}</CardTitle>
              </div>
        ...
        <CardContent className="space-y-4 relative z-10">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div className="info-card space-y-1 p-3 rounded-lg bg-white/5 backdrop-blur-sm border border-white/10">
              ...
              <p className="text-black/60">{formatDate(assignment.dueDate)}</p>
            </div>
            <div className="info-card space-y-1 p-3 rounded-lg bg-white/5 backdrop-blur-sm border border-white/10">
              <div className="flex items-center gap-2">
          ...
          <div className="flex space-x-2 pt-2">
            {(assignment.type === "exam" || assignment.type === "quiz") && assignment.status === "pending" && (
              <Button
            ...
            {assignment.status === "completed" && (
              <Button
                variant="outline"
                className="w-full flex justify-center items-center gap-2 button2 bg-theme-primary text-white py-3 rounded-lg hover:bg-opacity-90 transition-all duration-200 font-medium"
                onClick={(e) => handleButtonClick(e.currentTarget)}
              >
